...
for (let i = 0; i < user.length; i++) {
  let userType = user[i]
  if (userType.isAmbassador) {
    console.log(`${`${userType.name} ${userType.lastName}`} è ambassador`)
  } else {
    console.log(`${`${userType.name} ${userType.lastName}`} non è ambassador`)
  }
}
